...
def make_directory(prefix):
    ...
    dirs = ['calibration-curve', 'confusion-matrix', 'save-model', 'save-scaler', 'violin-plot', 'prob-df']
    for dir_str in dirs:
...
def classifier_train(nz, model_str_list, cuda_num, max_iter, load_data_dir, save_dir, message_dir):
    ...
    with open(os.path.join(message_dir, 'print-message.txt'), "w") as text_file:
...
def classifier_test(save_dir, test_save_dir, model_str_list, sdss_test_data):
    classifiers = ['random-forest', 'xgboost']
    for classifier in classifiers:
