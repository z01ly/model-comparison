...
def check_range(model_str):
    input_data = utils.stack_train_val(model_str)

    check_dir = os.path.join("./dim_meaning", model_str)
    if not os.path.exists(check_dir):
        os.makedirs(check_dir)
    else:
        print(f"Directory already exists: {check_dir}")

    with open(os.path.join("./dim_meaning", model_str, "range.txt"), "w") as text_file:
        text_file.write(f"global min: {np.min(input_data)}, global max: {np.max(input_data)} \n")
        text_file.write(f"\n")


    for i in range(32):
        with open(os.path.join("./dim_meaning", model_str, "range.txt"), "a") as text_file:
...
def main(model_str, model, gpu_id, use_cuda=True, dimension=32):
    ...
    for i in range(5):
        os.makedirs(os.path.join("./dim_meaning", model_str, f"vector_{i}"), exist_ok=True)
        ...
        for j in range(dimension):
            ...
            savefig_path = os.path.join("./dim_meaning", model_str, f"vector_{i}", f"dim{j}.png")
